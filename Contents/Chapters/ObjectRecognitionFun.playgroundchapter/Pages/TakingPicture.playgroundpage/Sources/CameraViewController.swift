    ...
    @IBAction func takePicture(_ sender: Any) {
        cameraManager?.captureImage { image, error in
            if let image = image {
                ...
                PlaygroundKeyValueStore.current["objectImage"] = .data(imageData)
                self.send(PlaygroundMessageFromLiveView.showObjectCameraSuccessMessage.playgroundValue)
...
extension CameraViewController: PlaygroundLiveViewMessageHandler {
    public func receive(_ message: PlaygroundValue) {
        ...
        switch message {
        case .startObjectCamera(desiredCameraPosition: let position, desiredFlashSetting: let desiredFlashSetting):
