    ...
    @IBAction func takePicture(_ sender: Any) {
        cameraManager?.captureImage { image, error in
            if let image = image {
                ...
                PlaygroundKeyValueStore.current["handwritingImage"] = .data(imageData)
                self.send(PlaygroundMessageFromLiveView.showHandwritingCameraSuccessMessage.playgroundValue)
...
extension CameraViewController: PlaygroundLiveViewMessageHandler {
    public func receive(_ message: PlaygroundValue) {
        ...
        switch message {
        case .startHandwritingCamera(desiredCameraPosition: let position, desiredFlashSetting: let desiredFlashSetting):
